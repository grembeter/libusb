...
int usbi_inc_fd_ref(struct pollfd *fds, unsigned int nfds)
{
	int n;
	usbi_mutex_static_lock(&fd_table_lock);
	for (n = 0; n < nfds; ++n) {
...
int usbi_dec_fd_ref(struct pollfd *fds, unsigned int nfds)
{
	int n;
	usbi_mutex_static_lock(&fd_table_lock);
	for (n = 0; n < nfds; ++n) {
		fd_table[fds[n].fd]->refcount--;
		if (fd_table[fds[n].fd]->refcount == 0)
		{
			free_fd(fd_table[fds[n].fd]);
